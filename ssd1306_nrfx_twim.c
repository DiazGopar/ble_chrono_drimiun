...
static const nrfx_twim_t m_twim_ssd1306 = NRFX_TWIM_INSTANCE(TWI_INSTANCE_ID);  
#else
...
ret_code_t twim_master_init(uint32_t scl, uint32_t sda)
{
        ...
        do {
                err_code = nrfx_twim_init(&twim, &config, NULL, NULL);
...
void SSD1306_command(uint8_t c)
{
        ...
        //m_xfer_done = false;
        //err_code = nrfx_twim_xfer(&twim, &xfer, 0);
        //APP_ERROR_CHECK(err_code);

        //while (m_xfer_done == false);

        err_code = nrfx_twim_tx(&twim,_i2caddr,dta_send,2,false);
...
#define SH1106_SETSTARTLINE 0x40
    ...
    SSD1306_command(SH1106_SETSTARTLINE | 0x0); // line #0
        ...
        for (uint16_t i=0; i<(SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT/8); i++) {
                ...
                //while (m_xfer_done == false);
                err_code = nrfx_twim_tx(&twim,_i2caddr,tmpBuf,sizeof(tmpBuf),false);
#else
                ret_code_t ret;
...
#define SH1106_SETSTARTLINE 0x40
    ...
    for ( i = 0; i < height; i++) {
      ...
      for( j = 0; j < 8; j++){
          ...
          //while (m_xfer_done == false);
          err_code = nrfx_twim_tx(&twim,_i2caddr,tmpBuf,sizeof(tmpBuf),true);
#else
          ret_code_t ret;
          ret = nrf_drv_twi_tx(&m_twi_master, _i2caddr, tmpBuf, sizeof(tmpBuf), false);
          APP_ERROR_CHECK(ret);
