...
void Adafruit_GFX_putstring(char* buffer)
{
        while (*buffer) {
                Adafruit_GFX_write((uint8_t)*buffer);
                buffer++;
        }
}

void Adafruit_GFX_puts(char* buffer)
{
        Adafruit_GFX_putstring(buffer);
        Adafruit_GFX_write('\n');
}